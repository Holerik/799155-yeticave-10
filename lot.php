...
//или после добавления лота или для просмотра лота
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $winner_name = "";
    $winner_id = 0;
    $sql = "SELECT winner_id, autor_id  FROM lots WHERE id = $lot_id";
    $result = $yetiCave->query($sql);
    if ($result) {
    ...
    if (isset($_GET['uname'])) {
        ...
        $sql = "SELECT name FROM users WHERE id = $winner_id";
        $result = $yetiCave->query($sql);
        if ($result) {
            $row = mysqli_fetch_assoc($result);
    ...
    $sql = "";
    if ($lot_id > 0) {
        $sql = "SELECT COUNT(*) FROM lots l WHERE l.id = $lot_id";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($now > $fin) {
            ...
            foreach ($history as $item) {
                if (Check_rate($yetiCave, $lot_row['id'], $item['price'])) {
